...
| X4 | Services layer defect sweep — registered the `GlobalErrorHandler` that was never provided (app shipped no user-facing error reporting) and deleted the dead duplicate `ErrorHandler` on `AppComponent`; fixed the `PullToRefreshService` listener leak (`fn.bind(this)` on both add and remove meant teardown removed references the browser had never seen) and added the missing `touchcancel` reset; made the handler survive non-object throws (`error.message` off `undefined` threw *inside* the error handler). New specs: ErrorHandler registration + classifier, and the listener-removal regression, threshold/cap/detach/cancel paths | Silent failure reporting, leaked listeners, masked errors | ⚠️ Specs written, test run still pending |
