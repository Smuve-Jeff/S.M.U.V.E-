...
if 'audioSession = inject(AudioSessionService);' not in content:
    insertion_point = content.find('private musicManager = inject(MusicManagerService);')
    if insertion_point != -1:
        end_of_line = content.find('\n', insertion_point) + 1
        content = content[:end_of_line] + '  public audioSession = inject(AudioSessionService);\n' + content[end_of_line:]

with open('src/app/studio/arrangement-view/arrangement-view.component.html', 'r') as f:
    html = f.read()
...
html = html.replace('togglePlay()', 'audioSession.togglePlay()')
html = html.replace('toggleRecord()', 'audioSession.toggleRecord()')

with open('src/app/studio/arrangement-view/arrangement-view.component.html', 'w') as f:
    f.write(html)
