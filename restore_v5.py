...
# 1. profile.types.ts - FULL RESTORATION WITH ALL PROPERTIES
...
# 2. user-profile.service.ts - RESTORE WITH ALL EXPORTS AND METHODS
...
# 3. database.service.ts - RESTORE WITH ALL METHODS
...
export class DatabaseService {
  ...
  async listProjects(userId: string): Promise<any[]> {
    ...
    if (userId && typeof navigator !== 'undefined' && navigator.onLine) {
      try { return await firstValueFrom(this.http.get<any[]>(`${this.API_URL}/projects/${userId}`, this.getHeaders())); }
      catch (error) { this.logger.error('Failed to list projects from cloud', error); }
    }
  ...
  async loadArtistIdentity(userId: string): Promise<ArtistIdentityState | null> {
    if (userId && typeof navigator !== 'undefined' && navigator.onLine) {
      try {
        const response = await firstValueFrom(this.http.get<{ identity: ArtistIdentityState }>(`${this.API_URL}/identity/${userId}`, this.getHeaders()));
        if (response?.identity) return response.identity;
      } catch (error) { this.logger.error('Failed to load artist identity from cloud', error); }
    }
  ...
  async listConnectorJobs(userId: string): Promise<any[]> {
    if (userId && typeof navigator !== 'undefined' && navigator.onLine) {
      try { return await firstValueFrom(this.http.get<any[]>(`${this.API_URL}/identity/${userId}/connectors`, this.getHeaders())); }
      catch (error) { this.logger.error('Failed to list connector jobs from cloud', error); }
    }
    const localItems = await this.localStorageService.getAllItems('sync_queue');
    return localItems.filter((item) => item.userId === userId && item.channel === 'connector');
  }
}
""")

# 4. login-confirmation.service.ts - CLEAN RESTORATION
