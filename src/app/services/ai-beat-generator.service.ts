...
export interface BassLine {
  notes: { pitch: string; octave: number; position: number; duration: number; velocity: number }[];
  pattern: string;
  style: string;
}
...
export class AiBeatGeneratorService {
  ...
  generateBeat(style: string, title?: string, bpm?: number): FullBeatArrangement {
    ...
    const arrangement = this.generateArrangement(genre, drums, bass, chords, melody);
  ...
  private generateDrums(genre: string, bpm: number, swing: number, profile: StyleProfile | null): DrumPattern {
    const isTrap = genre.toLowerCase().includes('trap') || genre.toLowerCase().includes('hip hop');
    const isPop = genre.toLowerCase().includes('pop');
    const isRock = genre.toLowerCase().includes('rock');
    const isElectronic = genre.toLowerCase().includes('electronic') || genre.toLowerCase().includes('house');
    ...
    if (isTrap || isRnB) {
      ...
      for (let i = 0; i < 16; i++) {
        if (i % 2 === 0) {
          hihat.push({ position: i, velocity: 0.5 + Math.random() * 0.3, probability: 0.9 });
        }
      }
      ...
      for (let i = 0; i < 16; i++) {
        const vel = i % 2 === 0 ? 0.5 + Math.random() * 0.2 : 0.3 + Math.random() * 0.2;
      ...
      for (let i = 0; i < 16; i++) {
        hihat.push({ position: i, velocity: i % 2 === 0 ? 0.7 : 0.4, probability: 0.9 });
      }
      ...
      for (let i = 0; i < 16; i++) {
        hihat.push({ position: i, velocity: 0.4 + Math.random() * 0.3, probability: 0.7 });
      }
      ...
      for (let i = 0; i < 16; i++) {
        ...
        hihat.push({ position: i + swingOffset, velocity: 0.3 + Math.random() * 0.2, probability: 0.6 });
      }
    } else {
      ...
      for (let i = 0; i < 16; i += 2) {
        hihat.push({ position: i, velocity: 0.4 + Math.random() * 0.2, probability: 0.8 });
      }
    }
    ...
    const matchedKey = Object.keys(patternNames).find(k => genre.toLowerCase().includes(k));
    const name = matchedKey ? patternNames[matchedKey] : 'Custom Groove';

    return { kick, snare, hihat, clap, percussion, bpm, swing, name, description: `${genre} drum pattern with ${swing > 0.5 ? 'heavy' : 'subtle'} swing feel` };
  }

  /** Generate bass line based on key and genre */
  private generateBass(key: string, genre: string, profile: StyleProfile | null): BassLine {
    const isTrap = genre.toLowerCase().includes('trap') || genre.toLowerCase().includes('hip hop');
    const bassStyle = profile?.productionCharacteristics?.bassStyle || '808 sub-bass with melodic lines';
    ...
    if (isTrap) {
      ...
      notes.push({ pitch: root, octave, position: 0, duration: 6, velocity: 1.0 });
      notes.push({ pitch: root, octave: octave + 1, position: 8, duration: 2, velocity: 0.8 });
      notes.push({ pitch: root, octave, position: 10, duration: 6, velocity: 0.6 });
    } else {
      // Standard bass: root-fifth pattern
      notes.push({ pitch: root, octave, position: 0, duration: 4, velocity: 1.0 });
      notes.push({ pitch: this.getFifth(root), octave, position: 4, duration: 2, velocity: 0.7 });
      notes.push({ pitch: root, octave, position: 8, duration: 4, velocity: 0.9 });
      notes.push({ pitch: this.getFifth(root), octave, position: 12, duration: 4, velocity: 0.6 });
    }
  ...
  private generateChords(key: string, genre: string, profile: StyleProfile | null): ChordStab[] {
    ...
    const chords: ChordStab[] = [
      { chord: selected.chords[0] || 'i', position: 0, duration: 4, velocity: 0.8, inversion: 'root' },
      { chord: selected.chords[1] || 'iv', position: 4, duration: 4, velocity: 0.7, inversion: 'root' },
      { chord: selected.chords[2] || 'v', position: 8, duration: 4, velocity: 0.75, inversion: 'root' },
      { chord: selected.chords[3] || 'iv', position: 12, duration: 4, velocity: 0.7, inversion: 'first' },
    ];
  ...
  private generateMelody(key: string, genre: string, profile: StyleProfile | null, chords: ChordStab[]): MelodyPhrase[] {
    const phrases: MelodyPhrase[] = [
      {
        notes: [
          { pitch: key.replace('m', ''), octave: 4, position: 0, duration: 2, velocity: 0.8 },
          { pitch: this.getThird(key), octave: 4, position: 4, duration: 2, velocity: 0.7 },
          { pitch: this.getFifth(key), octave: 4, position: 8, duration: 2, velocity: 0.75 },
          { pitch: this.getSeventh(key), octave: 4, position: 12, duration: 4, velocity: 0.6 },
        ],
        phrase: 'Main Hook',
        style: profile?.songwritingCharacteristics?.hookStyle || 'Melodic, repetitive',
      },
      {
        notes: [
          { pitch: this.getThird(key), octave: 4, position: 2, duration: 2, velocity: 0.6 },
          { pitch: key.replace('m', ''), octave: 4, position: 6, duration: 2, velocity: 0.65 },
          { pitch: this.getFifth(key), octave: 5, position: 10, duration: 2, velocity: 0.7 },
        ],
        phrase: 'Variation',
  ...
  private generateArrangement(genre: string, drums: DrumPattern, bass: BassLine, chords: ChordStab[], melody: MelodyPhrase[]): ArrangementSection[] {
    const isTrap = genre.toLowerCase().includes('trap') || genre.toLowerCase().includes('hip hop');
    const isPop = genre.toLowerCase().includes('pop');
    ...
    if (isTrap) {
      return [
        { name: 'Intro', bars: 8, elements: ['hihat', 'percussion'], energy: 0.3, description: 'Filtered hat pattern builds anticipation', transition: 'Riser FX + snare build' },
        { name: 'Verse', bars: 16, elements: ['kick', 'snare', 'hihat', 'bass'], energy: 0.6, description: 'Full drums + bass, minimal melody' },
        { name: 'Pre-Chorus', bars: 8, elements: ['all'], energy: 0.8, description: 'Add melody, energy builds', transition: 'Beat switch or fill' },
        { name: 'Chorus', bars: 16, elements: ['all'], energy: 1.0, description: 'Full arrangement, max energy', transition: 'Filter sweep down' },
        { name: 'Verse', bars: 16, elements: ['kick', 'snare', 'hihat', 'bass'], energy: 0.6, description: 'Drop back, second verse' },
        { name: 'Chorus', bars: 16, elements: ['all'], energy: 1.0, description: 'Full energy return' },
        { name: 'Bridge', bars: 8, elements: ['chords', 'melody'], energy: 0.5, description: 'Reduced elements, emotional core', transition: 'Build with riser' },
        { name: 'Outro', bars: 8, elements: ['hihat', 'melody'], energy: 0.3, description: 'Fade out with main hook' },
      ];
    }

    if (isPop) {
      return [
        { name: 'Intro', bars: 8, elements: ['hihat', 'chords'], energy: 0.3, description: 'Establish vibe with chords' },
        { name: 'Verse', bars: 16, elements: ['kick', 'snare', 'hihat', 'bass'], energy: 0.5, description: 'Rhythm section groove' },
        { name: 'Pre-Chorus', bars: 8, elements: ['all'], energy: 0.7, description: 'Building tension', transition: 'Snare roll build' },
        { name: 'Chorus', bars: 16, elements: ['all'], energy: 1.0, description: 'Full arrangement peaks' },
        { name: 'Verse', bars: 16, elements: ['kick', 'snare', 'hihat', 'bass'], energy: 0.5, description: 'Second verse' },
        { name: 'Chorus', bars: 16, elements: ['all'], energy: 1.0, description: 'Bigger production' },
        { name: 'Bridge', bars: 8, elements: ['chords', 'melody'], energy: 0.4, description: 'Emotional breakdown', transition: 'Key change riser' },
        { name: 'Outro', bars: 8, elements: ['hihat', 'melody'], energy: 0.3, description: 'Final fade' },
      ];
    }

    if (isElectronic) {
      return [
        { name: 'Intro', bars: 16, elements: ['hihat', 'percussion'], energy: 0.2, description: 'Filtered percussion builds' },
        { name: 'Build 1', bars: 8, elements: ['kick', 'hihat', 'percussion'], energy: 0.5, description: 'Add kick, tension rises', transition: 'Riser FX' },
        { name: 'Drop 1', bars: 16, elements: ['all'], energy: 1.0, description: 'Full power drop' },
        { name: 'Breakdown', bars: 16, elements: ['chords', 'melody'], energy: 0.3, description: 'Reduced for impact', transition: 'Filter sweep up' },
        { name: 'Build 2', bars: 8, elements: ['kick', 'hihat', 'bass'], energy: 0.7, description: 'Re-introduce rhythm', transition: 'Snare build' },
        { name: 'Drop 2', bars: 16, elements: ['all'], energy: 1.0, description: 'Second drop, varied elements' },
        { name: 'Outro', bars: 8, elements: ['chords'], energy: 0.2, description: 'Ambient fade' },
      ];
    }

    // Default structure
    return [
      { name: 'Intro', bars: 4, elements: ['hihat'], energy: 0.3, description: 'Establish groove' },
      { name: 'Verse', bars: 16, elements: ['kick', 'snare', 'hihat', 'bass'], energy: 0.5, description: 'Main section' },
      { name: 'Chorus', bars: 16, elements: ['all'], energy: 0.9, description: 'Peak section' },
      { name: 'Verse', bars: 16, elements: ['kick', 'snare', 'hihat', 'bass'], energy: 0.5, description: 'Second verse' },
      { name: 'Chorus', bars: 16, elements: ['all'], energy: 0.9, description: 'Return to peak' },
      { name: 'Outro', bars: 8, elements: ['hihat', 'melody'], energy: 0.2, description: 'Wind down' },
    ];
  }
  ...
  generateBeatBlueprint(style: string, title?: string): string {
    ...
    const sections = beat.arrangement.map(s =>
...
Use /beat [style] \"title\" to generate more beats.`;
  ...
  private generateProductionNotes(profile: StyleProfile | null, genre: string): string[] {
    const notes: string[] = [];
    
    if (profile) {
      notes.push(`Style reference: ${profile.artistName} — ${profile.productionCharacteristics.mixStyle}`);
      notes.push(`Signature element: ${profile.productionCharacteristics.signatureElement}`);
    }
    
    if (genre.toLowerCase().includes('trap') || genre.toLowerCase().includes('hip hop')) {
      ...
      notes.push('Keep vocal frequencies clear: cut 300Hz mud, boost 2-4kHz presence');
  ...
  private pickSwing(genre: string): number {
    ...
    if (genre.toLowerCase().includes('r&b') || genre.toLowerCase().includes('soul')) return 0.6;
    if (genre.toLowerCase().includes('hip hop') || genre.toLowerCase().includes('trap')) return 0.55;
    if (genre.toLowerCase().includes('pop')) return 0.5;
    if (genre.toLowerCase().includes('electronic') || genre.toLowerCase().includes('house')) return 0.3;
    return 0.5;
  }
