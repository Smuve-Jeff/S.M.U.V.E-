...
> = [
  ...
  },
  {
    ...
    outcome: 'At least three linked accounts under one consistent handle.',
    ...
    actions: [
      ...
      'Add each account to the identity record.',
    ],
    evidence: 'Three or more linked accounts on the artist identity.',
    effort: 'days',
    cost: 'free',
    requires: ['identity-name'],
    evaluate: (e) =>
      check(e.list(e.p.artistIdentity?.linkedAccounts).length >= 3, false, [
        'Link at least three of the artist’s own accounts under one handle.',
      ]),
  },
...
export class ArtistPathwayService {
  ...
  readout(profile: UserProfile | null | undefined): PathwayReadout {
    ...
    const nextAction =
      progress.find((entry) => entry.status === 'ready') ||
      progress.find((entry) => entry.status === 'in-progress') ||
      null;
   ...
   */
  destination(id: string | undefined): { label: string; url: string } | null {
    if (!id) return null;
  ...
  todayList(profile: UserProfile | null | undefined, limit = 3): string[] {
    const readout = this.readout(profile);
    const open = [...readout.steps].filter((entry) => entry.status !== 'complete');
    return open
      .sort((a, b) => this.openRank(a) - this.openRank(b))
      .slice(0, limit)
      .map((entry) => entry.step.title);
  }

  /**
   * Blocked steps sink below ready ones, but a partially finished step still
   * outranks a fresh one — finishing beats starting.
   */
  private openRank(entry: PathwayStepProgress): number {
    const areaIndex = AREA_ORDER.indexOf(entry.step.area);
    if (entry.status === 'in-progress') return areaIndex;
    if (entry.status === 'ready') return areaIndex + 100;
    return areaIndex + 200;
  }
