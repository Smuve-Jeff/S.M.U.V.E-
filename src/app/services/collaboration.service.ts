...
export class CollaborationService {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      this.social.sendPartyMessage(
        JSON.stringify({ type: 'PROJECT_SYNC', payload: snapshot })
      );
    });
    ...
    effect(() => {
      ...
      if (lastMsg && lastMsg.message.startsWith('{"type":"PROJECT_SYNC"')) {
        try {
          ...
          if (data.fromUserId !== this.social.currentPartyId()) {
            // Simple check
  ...
  sendProjectUpdate(sessionId: string, projectState: any): void {
    ...
    if (session && session.sessionId === sessionId) {
      this.social.sendPartyMessage(
        JSON.stringify({ type: 'PROJECT_SYNC', payload: projectState })
      );
    }
  }
