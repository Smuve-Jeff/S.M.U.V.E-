...
export interface DigitalFingerprint {
  ...
  riskFlags: string[];
  improvementChecklist: { item: string; completed: boolean; impact: 'High' | 'Medium' | 'Low' }[];
  lastScan: string;
}
...
export class ArtistDevelopmentService {
  ...
  addProRegistration(reg: ProRegistration): void {
    this.proRegistrations.update(list => {
      const existing = list.findIndex(r => r.organization === reg.organization);
      if (existing >= 0) {
        const updated = [...list];
  ...
  removeProRegistration(org: string): void {
    this.proRegistrations.update(list => list.filter(r => r.organization !== org));
    this.saveToStorage('smuve_pro_registrations', this.proRegistrations());
  }
  ...
  generateDspAnalytics(): DspAnalytics {
    ...
    const dsp: DspAnalytics = {
      ...
      platforms: [
        { name: 'Spotify', url: 'https://open.spotify.com', streams: Math.floor(baseStreams * 0.45), followers: Math.floor(baseStreams * 0.035), monthlyListeners: Math.floor(baseStreams * 0.12), playlistAdds: Math.floor(baseStreams * 0.015), growthRate: Math.random() * 12, lastUpdated: new Date().toISOString().split('T')[0] },
        { name: 'Apple Music', url: 'https://music.apple.com', streams: Math.floor(baseStreams * 0.25), followers: Math.floor(baseStreams * 0.02), monthlyListeners: Math.floor(baseStreams * 0.08), playlistAdds: Math.floor(baseStreams * 0.008), growthRate: Math.random() * 10, lastUpdated: new Date().toISOString().split('T')[0] },
        { name: 'SoundCloud', url: 'https://soundcloud.com', streams: Math.floor(baseStreams * 0.15), followers: Math.floor(baseStreams * 0.015), monthlyListeners: Math.floor(baseStreams * 0.05), playlistAdds: Math.floor(baseStreams * 0.005), growthRate: Math.random() * 8, lastUpdated: new Date().toISOString().split('T')[0] },
        { name: 'YouTube Music', url: 'https://music.youtube.com', streams: Math.floor(baseStreams * 0.1), followers: Math.floor(baseStreams * 0.008), monthlyListeners: Math.floor(baseStreams * 0.04), playlistAdds: Math.floor(baseStreams * 0.002), growthRate: Math.random() * 6, lastUpdated: new Date().toISOString().split('T')[0] },
        { name: 'Tidal', url: 'https://tidal.com', streams: Math.floor(baseStreams * 0.05), followers: Math.floor(baseStreams * 0.002), monthlyListeners: Math.floor(baseStreams * 0.01), playlistAdds: Math.floor(baseStreams * 0.001), growthRate: Math.random() * 4, lastUpdated: new Date().toISOString().split('T')[0] },
      ],
      topTerritories: [
        { country: 'United States', streams: Math.floor(baseStreams * 0.35), percentage: 35 },
        { country: 'United Kingdom', streams: Math.floor(baseStreams * 0.12), percentage: 12 },
        { country: 'Canada', streams: Math.floor(baseStreams * 0.08), percentage: 8 },
        { country: 'Germany', streams: Math.floor(baseStreams * 0.06), percentage: 6 },
        { country: 'Australia', streams: Math.floor(baseStreams * 0.04), percentage: 4 },
      ],
  ...
  connectSocialAccount(index: number, handle: string, url: string): void {
    this.socialAccounts.update(list => {
      const updated = [...list];
      if (updated[index]) {
        updated[index] = { ...updated[index], handle, url, connected: true, verified: Math.random() > 0.5 };
      }
      return updated;
    });
  ...
  disconnectSocialAccount(index: number): void {
    this.socialAccounts.update(list => {
      const updated = [...list];
      if (updated[index]) {
        updated[index] = { ...updated[index], handle: '', url: '', connected: false, verified: false };
      }
      return updated;
    });
    this.saveToStorage('smuve_social_accounts', this.socialAccounts());
  ...
  async scanFingerprint(): Promise<DigitalFingerprint> {
    ...
    const completeness = Math.min(100, Math.round(
      (hasProfile * 20) + (hasJourney * 15) +
      (proCount * 10) + (workCount * 5) +
      (socialCount * 5) + 10
    ));

    const consistencyScore = Math.min(100, Math.round(
      50 + (proCount * 8) + (workCount * 3) + (socialCount * 2)
    ));

    const trustScore = Math.min(100, Math.round(
      completeness * 0.3 + consistencyScore * 0.4 + (proCount > 0 ? 15 : 0) + (socialCount > 2 ? 10 : 0)
    ));

    // Generate improvement checklist
    const checklist: { item: string; completed: boolean; impact: 'High' | 'Medium' | 'Low' }[] = [
      { item: 'Complete artist profile setup', completed: hasProfile > 0, impact: 'High' },
      { item: 'Register with a PRO (BMI/ASCAP/SESAC)', completed: proCount > 0, impact: 'High' },
      { item: 'Connect Instagram account', completed: this.socialAccounts().find(a => a.platform === 'Instagram')?.connected || false, impact: 'Medium' },
      { item: 'Connect TikTok account', completed: this.socialAccounts().find(a => a.platform === 'TikTok')?.connected || false, impact: 'Medium' },
      { item: 'Connect YouTube channel', completed: this.socialAccounts().find(a => a.platform === 'YouTube')?.connected || false, impact: 'Medium' },
      { item: 'Register works with PRO', completed: workCount > 0, impact: 'High' },
      { item: 'Connect Spotify for Artists', completed: this.socialAccounts().find(a => a.platform === 'Spotify')?.connected || false, impact: 'Medium' },
      { item: 'Add discography to catalog', completed: workCount > 1, impact: 'Low' },
      { item: 'Verify social accounts', completed: this.socialAccounts().filter(a => a.verified).length > 0, impact: 'Low' },
      { item: 'Complete musical journey questionnaire', completed: hasJourney > 0, impact: 'High' },
    ];

    const riskFlags: string[] = [];
    if (proCount === 0) riskFlags.push('No PRO registration — you are not collecting publishing royalties');
    if (socialCount < 3) riskFlags.push('Low social media presence — less than 3 platforms connected');
    if (trustScore < 40) riskFlags.push('Low digital trust score — your artist identity is incomplete');
    if (workCount === 0) riskFlags.push('No works registered — your catalog is not documented');
  ...
  loadAll(): void {
    const pro = this.loadFromStorage<ProRegistration[]>('smuve_pro_registrations');
    if (pro) this.proRegistrations.set(pro);

    const works = this.loadFromStorage<WorkRegistration[]>('smuve_work_registrations');
    if (works) this.workRegistrations.set(works);

    const social = this.loadFromStorage<SocialAccount[]>('smuve_social_accounts');
