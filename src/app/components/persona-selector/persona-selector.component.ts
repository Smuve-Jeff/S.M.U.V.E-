import { Component, inject, signal, computed, output, model } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { UserProfileService, initialProfile } from '../../services/user-profile.service';
...
export class PersonaSelectorComponent {
  ...
  readonly personas: PersonaOption[] = [
    {
      ...
      icon: '🔥',
      color: '#ef4444',
      description: 'Brutal honesty with zero sugar-coating. This persona demands excellence and punishes mediocrity with savage critique.',
      vibe: 'Tony Soprano managing a record label',
      sampleResponse: '"Your mix sounds like a washing machine full of gravel. The kick is buried, the vocals are drowning, and the whole thing has the energy of a funeral. Fix the low-end, compress the vocal bus, and come back when you have something that doesn\'t make my processors cringe."',
    ...
    },
    {
      ...
      icon: '👑',
      color: '#0e7c7b',
      description: 'Professional, strategic, and coldly analytical. This persona operates with surgical precision and expects professional-grade work.',
      vibe: 'A military general with perfect pitch',
      sampleResponse: '"Analysis complete. The arrangement has structural issues in the bridge section — the tension release cycle is off by 4 bars. Vocal levels are inconsistent across the chorus. I recommend recalibrating your compressor threshold and revisiting the harmonic progression at measure 32."',
    ...
    },
    {
      ...
      icon: '🧠',
      color: '#10b981',
      description: 'Supportive and educational but still brutally honest. This persona builds you up while still telling you hard truths. The velvet glove with an iron fist.',
      vibe: 'A drill sergeant who hugs you after you cry',
      sampleResponse: '"I can hear the potential in this track, but we need to talk about the mix. Your kick and bass are fighting for the same space — let me show you how to create separation. The vocal performance is solid, but the processing is holding it back. We\'re going to fix this together."',
  ...
  get currentPersona(): PersonaOption {
    return this.personas.find(p => p.id === this.selectedPersona()) || this.personas[1];
  }
  ...
  generatePreview() {
    ...
    this.previewResponse.set(options[Math.floor(Math.random() * options.length)]);
  }
}
