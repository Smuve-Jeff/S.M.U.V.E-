...
import { StrategicTask, IntelligenceBrief, MarketAlert } from '../../types/ai.types';
...
export class StrategyHubComponent implements OnInit {
  ...
  getImpactColor(impact: string): string {
    switch (impact) {
      case 'Extreme': return 'text-brand-primary font-black';
  ...
  getSeverityClass(severity: string): string {
    switch (severity) {
      case 'Critical': return 'bg-brand-primary/20 text-brand-primary border-brand-primary/30';
