...
export class ImageVideoLabComponent implements OnDestroy {
  ...
  // Playback Signals
  playbackUrl = signal<string | null>(null);
  isPlaybackMode = signal(false);
  ...
  deleteClip(url: string) {
    this.recordedClips.update(clips => clips.filter(c => c.url !== url));
  ...
  playClip(url: string) {
    ...
    this.isPlaybackMode.set(true);
    this.aiFeedback.set('Playback Mode Engaged. Reviewing tactical capture.');
  }

  exitPlayback() {
    this.isPlaybackMode.set(false);
    this.playbackUrl.set(null);
    if (!this.isCameraActive()) {
      this.aiFeedback.set('S.M.U.V.E. Camera System Offline. Initialize for tactical visual capture.');
    }
  }
  ...
  ngOnDestroy() {
    this.stopCamera();
    this.recordedClips().forEach(clip => URL.revokeObjectURL(clip.url));
  }
}
