...
export class PianoRollComponent implements AfterViewInit {
  ...
  addNoteLocal(midi: number, step: number) {
    ...
    this.engine.triggerAttack(
      trackId,
      this.musicManager.midiToFreq(midi),
            ...
            notes: t.notes.map((n) => {
              if (this.selectedNoteIds().has(n.id)) {
                ...
                if ((this.musicManager as any).updateNote) {
                  (this.musicManager as any).updateNote(trackId, n.id, {
                    midi: newMidi,
                  });
                }
  ...
  arrangementBars() {
    return Array.from({ length: this.numMeasures }, (_, i) => ({
      index: i,
      noteCount:
        this.selectedTrack()?.notes.filter((n) => Math.floor(n.step / 16) === i)
          .length || 0,
    }));
  }
            ...
            notes: t.notes.map((n) => {
              if (this.selectedNoteIds().has(n.id)) {
                return {
                  ...
                  velocity: Math.max(
                    0.1,
                    Math.min(1, n.velocity + (Math.random() * 0.2 - 0.1))
                  ),
  ...
  duplicateSelected() {
    ...
    this.musicManager.tracks.update((tracks) =>
      tracks.map((t) => {
        if (t.id === trackId) {
          const selectedNotes = t.notes.filter((n) =>
            this.selectedNoteIds().has(n.id)
          );
      ...
      tracks.map((t) => {
        if (t.id === trackId) {
          return {
            ...
            notes: t.notes.map((n) =>
              this.selectedNoteIds().has(n.id)
                ? { ...n, length: Math.max(0.25, n.length + delta) }
                : n
            ),
          };
        }
