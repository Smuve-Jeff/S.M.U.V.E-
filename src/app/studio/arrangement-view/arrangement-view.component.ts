...
export class ArrangementViewComponent {
  ...
  addTrack(): void { this.musicManager.ensureTrack('grand-piano'); }
  selectTrack(trackId: number): void { this.musicManager.selectedTrackId.set(trackId); }
  isTrackSelected(trackId: number): boolean { return this.musicManager.selectedTrackId() === trackId; }
  toggleMute(trackId: number): void { this.musicManager.toggleMute(trackId); }
  toggleSolo(trackId: number): void { this.musicManager.toggleSolo(trackId); }
  onDragOver(event: DragEvent) { event.preventDefault(); }
  onDrop(event: DragEvent, trackId: number) { event.preventDefault(); }
}
