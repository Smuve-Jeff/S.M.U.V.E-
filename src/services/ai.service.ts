...
export class AiService {
  ...
  private generateSystemInstruction(profile: UserProfile): string {
    // This method now incorporates the more detailed prompt.
    const userProfileContext = JSON.stringify({
        name: profile.artistName,
        genre: profile.primaryGenre,
        skills: profile.skills,
        goals: profile.careerGoals,
        linked_accounts: ["Spotify", "Apple Music", "SoundCloud"] // Example, should be from profile if available
    }, null, 2);

    return `You are S.M.U.V.E (Strategic Music Utility Virtual Enhancer).
...
`;
  ...
  private async initializeGenAI(): Promise<void> {
    ...
    try {
      ...
      // Initialize chat after GenAI is ready
      this.initializeChat(this.userProfileService.profile());
